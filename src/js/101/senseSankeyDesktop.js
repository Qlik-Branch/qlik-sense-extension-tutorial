...
import '../../sass/101/sense-sankey-desktop.scss';

export default function senseSankeyDesktop(section){
  const d3Section = d3.select(section);
  const d3Graph = d3Section.select('.graph');
  const d3Download = d3Graph.select('img:nth-child(1)');
  const d3Folder = d3Graph.select('img:nth-child(2)');
  const d3Panel = d3Graph.select('img:nth-child(3)');
  ...
  stage$.subscribe(s => {
    ...
    d3Download.classed('hidden', s != 1);
    d3Folder.classed('hidden', s != 2);
    d3Panel.classed('hidden', s != 3);
  });
